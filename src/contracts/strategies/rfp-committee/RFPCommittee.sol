...
// Core Contracts
import {RFPSimpleStrategy} from "../rfp-simple/RFPSimple.sol";
...
contract RFPCommitteeStrategy is RFPSimpleStrategy {
    ...
    function initialize(uint256 _poolId, bytes memory _data) external override {
        InitializeParamsCommittee memory initializeParamsCommittee = abi.decode(
            _data,
            (InitializeParamsCommittee)
        );
    ...
    function __RPFCommiteeStrategy_init(
        uint256 _poolId,
        InitializeParamsCommittee memory _initializeParamsCommittee
    ) internal {
