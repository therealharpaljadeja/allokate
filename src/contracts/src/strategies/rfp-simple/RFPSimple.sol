...
contract RFPSimpleStrategy is BaseStrategy, ReentrancyGuard {
    ...
    function initialize(
        ...
        InitializeParams memory initializeParams = abi.decode(
            _data,
            (InitializeParams)
        );
    ...
    function __RFPSimpleStrategy_init(
        uint256 _poolId,
    ...
    function setMilestones(
        ...
        if (milestones.length > 0) {
            if (milestones[0].milestoneStatus != Status.None)
    ...
    function submitUpcomingMilestone(Metadata calldata _metadata) external {
        ...
        if (
            acceptedRecipientId != msg.sender &&
            !_isProfileMember(acceptedRecipientId, msg.sender)
        ) {
            revert UNAUTHORIZED();
        }
    ...
    function rejectMilestone(
        ...
        if (milestones[_milestoneId].milestoneStatus != Status.Pending)
            revert MILESTONE_NOT_PENDING();

        milestones[_milestoneId].milestoneStatus = Status.Rejected;

        emit MilestoneStatusChanged(
            _milestoneId,
            milestones[_milestoneId].milestoneStatus
        );
    }
    ...
    function _registerRecipient(
        ...
        //  @custom:data (address registryAnchor, address recipientAddress, uint256 proposalBid, Metadata metadata)
        (registryAnchor, recipientAddress, proposalBid, metadata) = abi.decode(
            _data,
        ...
        if (
            metadataRequired &&
            (bytes(metadata.pointer).length == 0 || metadata.protocol == 0)
        ) {
        ...
        onlyPoolManager(_sender)
    {
        ...
        // Decode the '_data'
        (acceptedRecipientId, finalProposalBid) = abi.decode(
            _data,
            (address, uint256)
        );
    ...
    function _distribute(
        ...
        uint256 amount = (recipient.proposalBid * milestone.amountPercentage) /
            1e18;
    ...
    function _isProfileMember(
        address _anchor,
        address _sender
    ) internal view returns (bool) {
        IRegistry.Profile memory profile = _registry.getProfileByAnchor(
            _anchor
        );
    ...
    function _getRecipient(
        address _recipientId
    ) internal view returns (Recipient memory recipient) {
        recipient = _recipients[_recipientId];

        if (
            acceptedRecipientId != address(0) &&
            acceptedRecipientId != _recipientId
        ) {
            recipient.recipientStatus = recipient.recipientStatus > Status.None
                ? Status.Rejected
                : Status.None;
        }
    }
