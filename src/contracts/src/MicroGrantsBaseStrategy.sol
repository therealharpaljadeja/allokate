...
abstract contract MicroGrantsBaseStrategy is BaseStrategy, ReentrancyGuard {
    ...
    function increaseMaxRequestedAmount(
        uint256 _maxRequestedAmount
    ...
    function _registerRecipient(
        ...
        //  @custom:data (address registryAnchor, address recipientAddress, uint256 requestedAmount, Metadata metadata)
        (registryAnchor, recipientAddress, requestedAmount, metadata) = abi
            .decode(_data, (address, address, uint256, Metadata));
    ...
    function _allocate(
        bytes memory _data,
        address _sender
    ) internal virtual override onlyActiveAllocation {
        (address recipientId, Status status) = abi.decode(
            _data,
            (address, Status)
        );
        Recipient storage recipient = _recipients[recipientId];
        ...
        if (
            recipientAllocations[recipientId][Status.Accepted] ==
            approvalThreshold
        ) {
            recipient.recipientStatus = Status.Accepted;
