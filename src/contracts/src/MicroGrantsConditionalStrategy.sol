...
contract MicroGrantsConditionalStrategy is MicroGrantsBaseStrategy {
    ...
    function _beforeAllocate(
        ...
        if (Token(requiredToken).balanceOf(_sender) < requiredTokenBalance)
            revert NOT_ELIGIBLE();
    }
}
